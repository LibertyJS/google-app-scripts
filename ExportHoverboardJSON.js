...
function normalizeSpeakerData_ (speakerData) {
     // format id
     speakerData.id = normalizeHeader_(speakerData.id);

     //compress socials into being a single socials key. 
     if (speakerData.twitter || speakerData.website) {
       speakerData.socials = [];
       if (speakerData.twitter) {
         speakerData.socials.push({
             "icon": "twitter",
             "link": `https://twitter.com/${speakerData.twitter}`,
             "name": "Twitter"
         });
       }
       if (speakerData.website) {
         speakerData.socials.push({
           "icon": "website",
           "link": speakerData.website,
           "name": "Website"
       });
       }
     }

     // format talk title so it can be referenced as a key from talks and schedule
     speakerData.sessions = [{
       id: normalizeHeader_(speakerData.sessions)
     }];
